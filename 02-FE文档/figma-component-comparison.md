...
| Figma 组件 | 类型 | 说明 |
|-----------|------|------|
| aspect-ratio | 布局 | 保持元素宽高比 | ✅ **已开发** |
| carousel | 轮播 | 图片/内容轮播组件 | ✅ **已开发** |
| command | 命令 | 搜索/命令面板（如 Cmd+K） | ✅ **已开发** |
| hover-card | 悬浮 | 悬浮预览卡片 | ✅ 已存在 |
| input-otp | 输入 | OTP 验证码输入框 | ✅ **已开发** |
| navigation-menu | 导航 | 导航菜单组件 | ✅ **已开发** |
| sidebar | 导航 | 侧边导航栏 | ✅ **已开发** |
| skeleton | 加载 | 骨架屏加载状态 | ✅ **已开发** |
| slider | 输入 | 滑块选择器 | ✅ **已开发** |
| sonner | 通知 | 轻量级通知提示（Toast） | ✅ **已开发** |
| toggle-group | 输入 | 多选切换组 | ✅ 已存在 |
| toggle | 输入 | 单个切换开关 | ✅ 已存在 |
